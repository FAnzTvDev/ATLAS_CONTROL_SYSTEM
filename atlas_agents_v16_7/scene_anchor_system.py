...
class SceneAnchorSystem:
    ...
    # V9 WARDROBE LOCKS - Ravencroft specific HEX codes (from Gold Standard)
    # These take priority over defaults for Ravencroft characters
    RAVENCROFT_WARDROBE_LOCKS = {
        "EVELYN RAVENCROFT": "navy-blue jacket #1B263B, white blouse #FFFFFF, dark indigo pants #06080A",
        "LADY MARGARET RAVENCROFT": "Victorian mourning black dress #0D0D0D, silver severe bun hair #C0C0C0",
        "ARTHUR GRAY": "charcoal suit #1A1A1A, silver-gray hair #C0C0C0, maroon tie #722F37",
        "CLARA BYRNE": "muted gray-blue top #4A5568, dark apron #1A1A1A",
        "DR. ELIAS WARD": "brown tweed jacket #5D4E37, cream shirt #F5F5DC, wire-frame spectacles",
        "LAWYER": "dark navy suit #2C3E50, crisp white shirt #FFFFFF, leather briefcase",
        "THE LAWYER": "dark navy suit #2C3E50, crisp white shirt #FFFFFF, leather briefcase",
    }
    ...
    def _get_character_wardrobe(self, character_name: str) -> str:
        """Get wardrobe description for a character from story bible or legacy locks."""
        char_upper = character_name.upper().strip()

        # PRIORITY 1: Check Ravencroft wardrobe locks (HEX codes for consistency)
        for locked_name, wardrobe in self.RAVENCROFT_WARDROBE_LOCKS.items():
    ...
    def enhance_prompt(self, shot: Dict, scene_id: Optional[str] = None) -> str:
        ...
        This is the key function that was MISSING from generate_first_frames!
        """
        ...
        # 1. LOCATION ANCHOR - Same room across all shots
        ...
        # 2. CHARACTER WARDROBE - Same costume for each character
        ...
        # 3. LIGHTING CONTINUITY - Same lighting style
        lighting = anchor.get("lighting", "")
        if lighting and lighting.lower() not in nano_prompt.lower():
            parts.append(f"Lighting: {lighting}")

        # 3B. V17.8: COLOR GRADE ANCHOR — Same grade for ALL shots in scene
        ...
        # 4. HARD NEGATIVES - Genre-appropriate bans
        genre = (self.story_bible.get("genre") or "drama")
        if isinstance(genre, list):
            genre = genre[0] if genre else "drama"
        genre_key = genre.lower().replace(" ", "_").replace("-", "_")
        ...
        # 5. FACE STABILITY - Critical for character consistency
        if characters:
