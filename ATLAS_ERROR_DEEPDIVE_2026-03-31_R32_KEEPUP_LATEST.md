# ATLAS ERROR DEEPDIVE — 2026-03-31 R32 (KEEP-UP LATEST)

**Session timestamp:** 2026-03-31T10:11:26Z
**Run number:** R32
**Prior report:** ATLAS_ERROR_DEEPDIVE_2026-03-31_R31_KEEPUP_LATEST.md
**System version:** V36.5 (CLAUDE.md) / V31.0 docstring (runner header — OPEN-005)
**Ledger age at snapshot:** 1d 1h 22m (last entry 2026-03-30T08:47:31)
...
| Category | Count | Delta vs R31 | Status |
|----------|-------|-------------|--------|
| **FALSE_POSITIVES RETRACTED** | 0 | = | None this session |
| **META-CHRONIC total** | 2 | = | OPEN-009 (**21st**) + OPEN-010 (**18th**) |
| ARCHITECTURAL_DEBT | 1 | = (OPEN-002) | **31st** report |
| STALE_DOC | 2 | OPEN-003 (**31st**), OPEN-005 (**29th**) | Cosmetic |
| **CONFIRMED_FIXED** | **23** | = | 23 confirmed — 0 regressions |
| **CODE CHANGES SINCE R31** | **0** | **= (idle)** | **Runner mtime unchanged at 23:46:04 EDT — 6th consecutive idle-code cycle post-R26** |
| **DATA CHANGES SINCE R31** | **0** | = | shot_plan.json mtime unchanged (20:22:12 EDT) |
| **GENERATION SINCE R31** | **0 frames, 0 videos** | = | **System idle — 13th consecutive idle generation report (R20–R32)** |

**Key findings R32:**

1. 🟢 **NO REGRESSIONS. ALL 23 CONFIRMED-FIXED ITEMS INTACT.** Session enforcer: ✅ SYSTEM HEALTHY. Learning log: 0 regressions (ALL CLEAR, 22 fixes verified). Wire-A=6 hits, Wire-C=6 hits (12 combined). Wire-B logic at runner:5700. V37 governance: `_V37\|v37` refs=31 in runner (non-blocking hooks), /api/v37 endpoints=7 in orchestrator. CPC count=4. All canonical line positions stable — **6th consecutive idle-code cycle**.

2. 🟢 **ALL CANONICAL LINE POSITIONS STABLE (R31 → R32).** Runner total lines: 6,179 (unchanged). `_fail_sids` at runner:5700, `enrich_shots_with_arc` call at runner:4933, `isinstance` guard at runner:1512, `ACTIVE_VIDEO_MODEL` at runner:535, `LTX_FAST` guard at runner:505. **6th consecutive idle-code cycle post-R26.**

3. 🔴 **OPEN-009 META-CHRONIC: 21st consecutive report (R12→R32).** 4 shots (008_E01/E02/E03/M03b) still have `/api/media?path=` prefix in `video_url`. No data patch applied. ~2 min edit remains unapplied.

4. 🔴 **OPEN-010 META-CHRONIC: 18th consecutive report (R15→R32).** 4 shots (001_M02/M03/M04/M05) still have ghost `first_frame_url` pointing to non-existent files, all APPROVED. No operator action since META-CHRONIC escalation at R24.

5. 🟡 **SYSTEM IDLE (GENERATION) — 13th consecutive idle report (R20–R32).** No new frames or videos. Ledger age: 1d 1h 22m (+61m from R31). Average cycle cadence: ~60.1m/cycle (consistent).

6. 🟡 **006_M02/M04 AWAITING_APPROVAL (12th consecutive report, R21–R32).** 008_M03b + 008_M04 REGEN_REQUESTED (unchanged since R21). No operator action.

7. 🟡 **REWARD SIGNAL FROZEN — 31st consecutive report.** 228 ledger entries (unchanged). 87.8% heuristic I=0.75 (36/41 last-entry-per-shot). 5 real-VLM shots unchanged: 008_M01(1.0)/M02(0.9)/M04(0.8), 004_M01(1.0)/M02(1.0). Self-resolves on next generation run. **Ledger now 1d 1h+ stale.**

---

## 2. ORGAN HEALTH MAP

| Organ | Status | Signal | Proof (R32) |
|-------|--------|--------|------------|
| 🦴 Skeleton (shot_plan) | 🟢 HEALTHY | 97 shots. Bare-list guard at runner:1512. 97/97 arc positions. 62/62 M-shots with `_chain_group`. shot_plan.json mtime 20:22:12 EDT (unchanged from R31). | `isinstance(sp, list)` at runner:1512 ✅; runner:4506 guard ✅ |
...
| 🛡️ Immune (doctrine) | 🟡 DEGRADED | Runner header line 24 claims Seedance v2.0 PRIMARY (STALE_DOC — 29th). `_LTXRetiredGuard` error says "Use Seedance" (retired V31.0). Code correct (`ACTIVE_VIDEO_MODEL="kling"` at runner:535). CLAUDE.md V36.5 accurate. | runner:24 Seedance claim confirmed ✅; runner:535 → `"kling"` ✅ |
| ⚡ Nervous (hooks) | 🟢 HEALTHY | Session enforcer: ✅ SYSTEM HEALTHY. Learning log: 0 regressions (22 fixes). Wire-A (6 hits), Wire-C (6 hits), `_fail_sids` at runner:5700. `enrich_shots_with_arc` imported at runner:65, called at runner:4933. | Session enforcer R32 ✅ |
| 👁️ Eyes (vision/identity) | 🔴 SICK | All 5 env keys PRESENT. Vision backends: 4 available (gemini_vision + openrouter + florence_fal + heuristic per session_enforcer). OPEN-009: 4 API-path video_urls (21st). OPEN-010: 4 ghost first_frame_urls (18th). | Shot plan scan ✅; OPEN-009 + OPEN-010 confirmed unresolved |
| 🧠 Cortex (reward signal) | 🟡 DEGRADED | 228 entries (unchanged, **1d 1h 22m stale**). 87.8% heuristic I=0.75 (last-entry-per-shot). 5 real-VLM shots: 008_M01(1.0)/M02(0.9)/M04(0.8), 004_M01(1.0)/M02(1.0). | Ledger scan R32 ✅ — identical to R31 |
| 🎬 Cinematographer (generation) | 🟡 DEGRADED | 62 JPGs in first_frames/, 62 MP4s in videos_kling_lite/ (both unchanged). 6 scene_lite.mp4 files confirmed. OPEN-010: 001_M02-M05 ghost frames (META-CHRONIC). OPEN-009: 4 API-path video_urls. 2 AWAITING_APPROVAL, 2 REGEN_REQUESTED. run_report: success=True, errors=[]. | File counts + shot_plan scan R32 ✅ |
...
| 🔄 Regenerator (frozen fix) | 🟢 HEALTHY | Wire-A: 6 hits (lines 2507/2511/2516/2532/2536/2539). Wire-C: 6 hits (lines 5459/5479/5481/5484/5486/5488). Runner stable — no modification since R26 expansion (6th idle-code cycle). | `grep -c "WIRE-A\|WIRE-C"` → 12 ✅ |
| 📋 Doctrine Doc | 🟡 DEGRADED | Runner header line 24 Seedance claim persists (29th). LTX guard error message says "Use Seedance" (persists). CLAUDE.md V36.5 correct. | Confirmed via `sed -n '24p'` and lines 498-508 R32 ✅ |

---

## 3. CONFIRMED FIXED (DO NOT RE-INVESTIGATE)

23 items total — all intact, 0 regressions confirmed R32. Identical to R26/R27/R28/R29/R30/R31.
...
✅ **LEARNING LOG** — 0 regressions (ALL CLEAR — R32 confirmed, 22 fixes verified).
...
✅ **Wire-C WIRED** — 6 Wire-C hits confirmed R32 (lines 5459/5479/5481/5484/5486/5488).
...
### ⏱️ META-CHRONIC (21 reports): OPEN-009 — API-Path Prefix in video_url
...
**R32 status vs R31:** UNCHANGED. No data patch applied. Runner mtime unchanged — no code change. shot_plan.json mtime unchanged (20:22:12 EDT).

**META-CHRONIC: 21st consecutive report (R12→R32).**

**PROOF RECEIPT (R32 live):**
```
PROOF: python3 scan → shots where '/api/media' in video_url
OUTPUT: ['008_E01', '008_E02', '008_E03', '008_M03b']
EXACT VALUES (unchanged from R31):
...
**Classification:** META-CHRONIC (21st report). Data hygiene. ~2 min fix.

---

### ⏱️ META-CHRONIC (18 reports): OPEN-010 — Ghost first_frame_url on 001_M02-M05

**Issue:** 4 shots (001_M02/M03/M04/M05) have `first_frame_url` pointing to files that do not exist on disk. All 4 carry `_approval_status=APPROVED`. Escalated to META-CHRONIC at R24 (10-report threshold). 18th consecutive report.

**R32 status vs R31:** UNCHANGED. No operator action. shot_plan.json mtime unchanged. Runner stable.

**META-CHRONIC STATUS:** 18 consecutive reports (R15→R32). META-CHRONIC since R24.
...
CONFIRMS: Identical to R15→R31. 18th consecutive confirmation.
```
...
**Classification:** META-CHRONIC (18th report). Process failure — requires operator action.

---

### OPEN-002 (ARCHITECTURAL_DEBT — **31st consecutive report**)

**Issue:** Reward signal degradation — 87.8% heuristic I-scores (last-entry-per-shot basis). Pattern is self-resolving on next generation run. Ledger now **1d 1h 22m stale** — over 25 hours since last entry.

**PROOF RECEIPT (R32 live):**
```
PROOF: python3 last-entry-per-shot I-score analysis + ledger age
OUTPUT:
  TOTAL_ENTRIES: 228 (unchanged from R31)
  UNIQUE_SHOTS: 41
  LAST_I_HEURISTIC (0.75): 36/41 = 87.8% (87%)
  LAST_I_REAL_VLM: 5/41 — [('008_M01',1.0),('008_M02',0.9),('008_M04',0.8),('004_M01',1.0),('004_M02',1.0)]
  LEDGER_AGE: 1d 1h 22m (+61m from R31, >25 hours stale)
CONFIRMS: No new generation. Identical distribution to R31.
```

**Classification:** ARCHITECTURAL_DEBT (31st report). Resolves on next generation run. Vision backends: 4 available confirmed via session_enforcer.

---

### OPEN-003 (STALE_DOC — **31st consecutive report**)

**Issue:** No `[WIRE-B]` label in atlas_universal_runner.py. `_fail_sids` logic is functional at runner:5700 (confirmed stable R32 — line position unchanged from R26).

**PROOF (R32 live):**
```
PROOF: grep -c "WIRE-B" atlas_universal_runner.py → 0
       grep -n "_fail_sids" atlas_universal_runner.py → runner:5700 ✅
CONFIRMS: WIRE-B label absent. Logic intact. Line position stable at 5700 (7th consecutive confirmation since R26 baseline).
```

**Fix:** Add `# ── [WIRE-B] Quality gate stitch filter` comment at runner:5700. One line.

**Classification:** STALE_DOC. 31st consecutive report. Logic functional. Cosmetic only.

---

### OPEN-005 (STALE_DOC — **29th consecutive report**)
...
CONFIRMS: Both stale docstring references persist unchanged from R31. Code behavior correct.
```

**Classification:** STALE_DOC. 29th consecutive report. Code behavior correct.

---

### PRODUCTION_GAP — 006_M02 + 006_M04 AWAITING_APPROVAL (12th consecutive report, R21–R32)
...
OUTPUT:
  AWAITING_APPROVAL: ['006_M02', '006_M04'] — 12th consecutive report
...
**Classification:** PRODUCTION_GAP (operator action pending). Not a code bug. 12th consecutive report.

---

## 5. FALSE POSITIVES RETRACTED THIS SESSION

None. All open issues confirmed as reported in R31.

---

## 6. NEW OBSERVATIONS (R32 only)

### 6.1 Thirteenth Consecutive Idle Generation Report — R20 Through R32

System has produced zero new frames or videos across 13 consecutive keep-up cycles (R20–R32). All wires intact, all 23 confirmed-fixed items verified, 0 regressions, session enforcer HEALTHY. The two META-CHRONIC data patches (~12 min combined) remain unapplied for the 21st and 18th cycle respectively.

### 6.2 Ledger Now 25+ Hours Stale (R32)

The reward ledger last entry (2026-03-30T08:47:31) is now 1d 1h 22m old — over 25 hours without generation. This is a PRODUCTION_GAP milestone continuation, not a code failure. All vision backends remain available (4 confirmed) and will fire on next generation run.

### 6.3 Six Consecutive Idle Code Cycles (R27→R32)

Runner mtime (23:46:04 EDT 2026-03-30) and orchestrator mtime (15:43:25 EDT 2026-03-30) are identical across R27, R28, R29, R30, R31, and now R32 — **six consecutive reports with zero code modification.** Canonical wire positions stable across seven consecutive confirmations (R26 baseline → R32):

| Wire | Line (R26) | R27 | R28 | R29 | R30 | R31 | **R32** | Status |
|------|-----------|-----|-----|-----|-----|-----|---------|--------|
| `_fail_sids` (Wire-B logic) | 5700 | 5700 | 5700 | 5700 | 5700 | 5700 | **5700** | ✅ Stable |
| `enrich_shots_with_arc` call | 4933 | 4933 | 4933 | 4933 | 4933 | 4933 | **4933** | ✅ Stable |
| `isinstance` guard | 1512 | 1512 | 1512 | 1512 | 1512 | 1512 | **1512** | ✅ Stable |
| `ACTIVE_VIDEO_MODEL` | 535 | 535 | 535 | 535 | 535 | 535 | **535** | ✅ Stable |
| `LTX_FAST` guard | 505 | 505 | 505 | 505 | 505 | 505 | **505** | ✅ Stable |

### 6.4 Ledger Staleness Trend (13-Report View)
...
Cadence: 59–61m per cycle. Automated keep-up running on schedule. Average cadence: ~60.1m/cycle. 13th cycle.

### 6.5 META-CHRONIC Persistence Summary (Updated R32)

| Issue | First Appeared | META-CHRONIC Since | Consecutive Reports |
|-------|---------------|-------------------|---------------------|
| OPEN-009 (video_url patch) | R12 | R12 | **21 (R12→R32)** |
| OPEN-010 (ghost frames regen) | R15 | R24 | **18 (R15→R32; META-CHRONIC from R24)** |
...
|----------|-------|--------|-----------|----------|
| **P1** | OPEN-009 (video_url patch — 4 fields) | 2 min | Strip `/api/media?path=` from `video_url` on 008_E01/E02/E03/M03b in shot_plan.json | NO — data hygiene, **21st META-CHRONIC** |
...
| **P3** | OPEN-002 (reward signal) | 0 min code | Run generation — vision_judge fires with Gemini/OpenRouter | NO — observational; ledger now >25h stale |
...
```
□ Run session_enforcer → ✅ SYSTEM HEALTHY (confirmed R32: HEALTHY, 0 blocks)
□ ✅ OPEN-004 fixed — CPC decontamination wired (count=4 confirmed R32)
...
□ Verify vision backends online: gemini_vision + openrouter (confirmed R32 via session_enforcer: HEALTHY)
...
□ Canonical runner line positions (stable from R26, verified R27/R28/R29/R30/R31/R32): _fail_sids=5700, enrich_shots_with_arc=4933, isinstance=1512, ACTIVE_VIDEO_MODEL=535, LTX_FAST guard=505.
```
...
| Signal | R31 | R32 | Delta | Note |
|--------|-----|-----|-------|------|
| **META-CHRONIC count** | 2 | 2 | = | OPEN-009 (**21st**) + OPEN-010 (**18th**) |
| **OPEN-009 consecutive** | 20 | **21** | **+1** | No patch applied |
| **OPEN-010 consecutive** | 17 | **18** | **+1** | No operator action |
| **OPEN-002 consecutive** | 30 | **31** | **+1** | ARCHITECTURAL_DEBT |
| **OPEN-003 consecutive** | 30 | **31** | **+1** | STALE_DOC |
| **OPEN-005 consecutive** | 28 | **29** | **+1** | STALE_DOC |
| **PRODUCTION_GAP (AWAITING)** | 11th | **12th** | **+1** | 006_M02/M04 + 008_M03b/M04 |
| **Idle generation reports** | 12th (R20-R31) | **13th (R20-R32)** | **+1** | 0 new frames/videos |
| **Idle code cycles** | 5th | **6th** | **+1** | Runner/orchestrator mtimes unchanged |
| **Ledger age** | 24h21m | **25h22m** | **+61m** | >25h stale |
...
- **Prior report:** ATLAS_ERROR_DEEPDIVE_2026-03-31_R31_KEEPUP_LATEST.md
- **This report:** ATLAS_ERROR_DEEPDIVE_2026-03-31_R32_KEEPUP_LATEST.md
- **Next report:** ATLAS_ERROR_DEEPDIVE_2026-03-31_R33_KEEPUP_LATEST.md (expected ~2026-03-31T11:11Z)
...
```json
{
  "session_timestamp": "2026-03-31T10:11:26Z",
  "ledger_age_hours": 25.37,
  "run_number": 32,
  "delta_from_prior": "+61m",
  "idle_generation_cycles": 13,
  "idle_code_cycles": 6,
  "chronic_issues": [
    {
      "id": "OPEN-009",
  ...
  "architectural_debt": [
    {
      ...
      "description": "87.8% heuristic I-scores, ledger 25h+ stale — self-resolves on next generation run"
    }
  ],
  "stale_docs": [
    {"id": "OPEN-003", "consecutive_reports": 31, "description": "No [WIRE-B] label at runner:5700 — logic functional"},
    {"id": "OPEN-005", "consecutive_reports": 29, "description": "Runner header Seedance claim + LTX error message stale — code correct"}
  ],
